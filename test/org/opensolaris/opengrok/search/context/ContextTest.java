...
public class ContextTest {
    ...
    private void testGetContext(boolean limit, boolean hitList)
    	...
    	/**
    	 * added bold tags in text to be searched to 
    	 * ensure they are not being affected 
    	 * Bug Id: 1896281
    	 */
    	StringReader in = new StringReader("<b>abc</b> def ghi\n");
    ...
    /**
     * Test that full search has matched words in context.
     *
     * @throws ParseException
     */
    @Test
    public void fullSearchContextTest() throws ParseException {
        //accent match is returned in context
        searchContextTestHelper("`one two three", "`one", "`");
        searchContextTestHelper("`one two three", "`one", "one");
        searchContextTestHelper("one() two three", "\\(", "(");
        searchContextTestHelper("one() two three", "\\)", ")");
        searchContextTestHelper("one<> two three", "<", "&lt;");
    	searchContextTestHelper("one<> two three", ">", "&gt;");
    	searchContextTestHelper("one/ two three", "\\/", "/");
