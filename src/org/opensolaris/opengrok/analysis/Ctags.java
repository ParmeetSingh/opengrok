...
public class Ctags {
    ...
    //extra process builder added to find languages supported by ctags
    private ProcessBuilder langSupportedBuilder;
    
    /**
     * customExtensionsOptionsList will contain ctags options 
     * to include all new extensions in this format --langmap=java:+.j
     */
    private List<String> customExtensionsOptionsList;
    ...
    private void getCustomExtensionsOptionsList(){
    	
    	RuntimeEnvironment env = RuntimeEnvironment.getInstance();
    	
    	/**
         * running ctags --list-languages command to find out languages
         * supported by ctags 
         * langSupportBuilder is a ProcessBuilder object used to run commands 
         */
        if (langSupportedBuilder == null){
        	List<String> command = new ArrayList<>();
        	command.add(binary);
        	command.add("--list-languages");
        	
        	langSupportedBuilder = new ProcessBuilder(command);
        }
    	
    	customExtensionsOptionsList = new ArrayList<>();
        Set<String> ctagsLanguages=null;
		try {
			ctagsLanguages = getCtagsLanguageSupported();
		} catch (Exception e) {
			LOGGER.severe("Unable to fetch languages supported by Ctags.");
		}
		//getting added extensions from configuration POJO
		Configuration cfg = env.getConfiguration();
        List<ExtensionGroup> extGroups =  cfg.getAnalyzerMapping();
        if(ctagsLanguages!=null && extGroups!=null){
        	for(ExtensionGroup extGrp:extGroups){
            	if(ctagsLanguages.contains(extGrp.getName())){
            		for(String extension:extGrp.getExtList())
            			customExtensionsOptionsList.add("--langmap="+extGrp.getName()+":+."+extension);
            	}else
            		LOGGER.warning("The language"+extGrp.getName()+"is not supported by CTags");
    ...
    private void initialize() throws IOException{
        ...
        if(customExtensionsOptionsList==null)getCustomExtensionsOptionsList();
        ...
        if (processBuilder == null) {
            ...
            if(customExtensionsOptionsList!=null){
            	for(String customExtensionOpt:customExtensionsOptionsList){
                	command.add(customExtensionOpt);
                }
            }
    ...
    /**
     * extracting the output for the command ctags --list-languages
     * @return
     * @throws IOException
     * @throws InterruptedException
     */
    private Set<String> getCtagsLanguageSupported() throws IOException, InterruptedException {
    	Process process = langSupportedBuilder.start();
		int errCode = process.waitFor();
		if(errCode!=0)
			LOGGER.severe("Ctags languages list command executed gave error code "+errCode);
		return prepareCtagsLanguageList(process.getInputStream());
	}
    
    private  Set<String> prepareCtagsLanguageList(InputStream inputStream) throws IOException {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				//lower casing and removing all spaces in the line
				langList.add(line.toLowerCase().replaceAll("\\s+", "").trim());
			}
		} finally {
			br.close();
		}
