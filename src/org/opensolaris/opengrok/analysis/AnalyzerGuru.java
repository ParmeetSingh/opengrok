...
public class AnalyzerGuru {
    ...
    /**
     * contains a mapping of file extension group names and corresponding analyzer Factory class
     */
    private static final Map<String, FileAnalyzerFactory> nameAnalyzerMapping = new TreeMap<>();
    /**
     * custom map containing list of extension groups to be shown to user on the UI
     * key of map will be lower cased extension group Name
     * value will be as shown to the user on the UI
     */
    private static final Map<String, String> customizedFileTypeDescriptions = new TreeMap<>();
    
    /**
     * customFileTypeGroups will contain a mapping of file extensions 
     * against a list of all custom group names
     * for example file groups grind1 and grind2 mapped to
     * pl,seq and java,seq means pl will be mapping to grind1 
     * seq will be mapping to grind1,grind2
     * this is later used in populateDocument function 
     */
    private static final Map<String, List<String>> customFileTypeGroups = new TreeMap<>();
    
    
    private static boolean invalidArguments = false;
    private static AnalyzerException analyzerException;
    ...
    static {
        ...
        for (FileAnalyzerFactory analyzer : analyzers) {
            if (analyzer.getName() != null && !analyzer.getName().isEmpty()) {
                ...
                customizedFileTypeDescriptions.put(analyzer.getAnalyzer().getFileTypeName(), analyzer.getName());
                nameAnalyzerMapping.put(analyzer.getAnalyzer().getFileTypeName(), analyzer);
            }
        }
        
        try{
        	populateCustomizedFileTypeDescriptions();
        }catch(AnalyzerException ex){
        	analyzerException = ex;
    ...
    public static boolean checkInvalidArguments(){
    	return invalidArguments;
    }
    
    public static AnalyzerException getAnalyzerException(){
    	return analyzerException;
    }
    
    public static void populateCustomizedFileTypeDescriptions() throws AnalyzerException{
    	
    	Map<String, String> typeGroups = AnalyzerGuru.customizedFileTypeDescriptions;
    	Map<String, List<String>> customGroupsMap = AnalyzerGuru.customFileTypeGroups;
    	
    	//get the configuration from runtimeenvironment
    	Configuration cfg = RuntimeEnvironment.getInstance().getConfiguration();
    	List<String> excludeGroups = cfg.getExcludeGroups();
    	
    	//remove groups to be excluded from types menu in the UI
    	if(excludeGroups!=null){
    		for(String exlGrp:excludeGroups){
    			//fileTypeDescrpition has keys all in lower case
    			//and values are UI freindly i.e case sensitive
    			exlGrp = exlGrp.toLowerCase(); 
    			if(typeGroups.get(exlGrp)==null)
    				throw new AnalyzerException(ANALYZER_EXCEPTION_TYPE.NON_EXISTING_GROUP,
    							"Failed to exclude group: "+exlGrp+" from fileTypeDescriptions as it does not exist. "
    									+ "Try removing the group "+exlGrp+" from excludeGroups section in the extension mapping file."
    											+ "Non Existing groups cannot be removed.");
    			typeGroups.remove(exlGrp);
        	}
    	}
    	
    	//map newly added extensions to their analyzers
		List<ExtensionGroup> analyzerMapping = cfg.getAnalyzerMapping();
    	if(analyzerMapping!=null){
        	for(ExtensionGroup extGroup:analyzerMapping){
        		//converting the groups specified in lower case
        		String name = extGroup.getName().toLowerCase();
        		if(typeGroups.get(name)==null)
    				throw new AnalyzerException(ANALYZER_EXCEPTION_TYPE.NON_EXISTING_GROUP,
    							"Failed to add extensions to Analyzer Group name"+name+" as it does not exist."
    									+ "Verify the group name"+name+"in the analyzerMapping section of the extension mapping file.");
        		for(String extension:extGroup.getExtList()){
        			//extensions stored in ext map are in upper case
        			extension = extension.toUpperCase();
        			FileAnalyzerFactory analyzerFactory = ext.get(extension);
        			if(analyzerFactory!=null)
        				throw new AnalyzerException(ANALYZER_EXCEPTION_TYPE.EXTENSION_ALREADY_EXISTS,
        						"The extension "+extension+" has already been mapped to "+analyzerFactory.getName()+" analyzer."
        								+ "You cannot add a default extension to an analyzer.Please remove extension:"
        								+ extension+" from the analyzerMapping section of the extension mapping file."
        										+ "Only new extensions can be added to pre-existing groups"
        						);
        			addExtension(extension, nameAnalyzerMapping.get(name));
        		}
        	}
    	}

    	//adding custom groups
    	List<ExtensionGroup> customGroups = cfg.getCustomGroups();
    	if(customGroups!=null){
    		for(ExtensionGroup extGroup:customGroups){
        		String name = extGroup.getName();
        		String lowerCaseName = name.toLowerCase();
        		//populating in customized map for it show up in the UI
        		if(fileTypeDescriptions.get(lowerCaseName)!=null)
        			throw new AnalyzerException(ANALYZER_EXCEPTION_TYPE.GROUP_ALREADY_EXISTS,"Analyzer group "+name+" already exists."
        					+ "Please remove the group "+name+"from the customGroups section in the extension mapping file.");
        		typeGroups.put(lowerCaseName, name);
        		List<String> extList = extGroup.getExtList();
        		for(String extension:extList){
        			extension = extension.toUpperCase();
        			if(ext.get(extension)==null)
        				throw new AnalyzerException(ANALYZER_EXCEPTION_TYPE.NON_EXISTING_EXTENSION,
        								"The extension "+extension+" cannot be added to group :"+name+" because it does not exist."
        										+ "Default and new mapped extensions can only be added in the customGroups section "
        										+ "of the extension mapping file");
        			if(customGroupsMap.get(extension)==null){
        				customGroupsMap.put(extension, new ArrayList<String>());
        			}
        			List<String> groupList = customGroupsMap.get(extension);
        			groupList.add(name);
        		}
        	}
    	}
    ...
    public static Map<String, List<String>> getCustomFileTypeGroups() {
        return customFileTypeGroups;
    }

	public static Map<String, String> getCustomizedFileTypeDescriptions() {
        return customizedFileTypeDescriptions;
    }
    ...
    public void populateDocument(Document doc, File file, String path,
        ...
        if (fa != null) {
            ...
            /**
             * customFileTypeGroups has extension 
             * vs custom file groups mapping.
             * We will add all custom file type groups to the document file.
             */
            String suffix = FilenameUtils.getExtension(file.getName()).toUpperCase(Locale.getDefault());
            List<String> grpList = customFileTypeGroups.get(suffix);
          
            if(grpList!=null){
            	for(String grp:grpList){
                	doc.add(new StringField(QueryBuilder.TYPE, grp, Store.YES));
                }
            }
